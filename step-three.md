...
Before you actually run the job, it's important to understand the S3 bucket and folder structure so that proper IAM policies can be applied to S3 bucket
```
<your-S3-bucket>
|- ds-csv
|-- nyc-tlc
|- ds-pq (store the parquet files that the job will emit as output of this job)
|- jobs
|-- scripts
|-- temp
```
...
![Run job](screenshots/job-history.png)
